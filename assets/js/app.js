...
const itemImages = (item, fallback='assets/images/product-tshirt.svg') => {
  const values = Array.isArray(item?.images) ? item.images : [];
  const combined = [...values, item?.image].filter(Boolean).map(value => safeUrl(value));
  return [...new Set(combined)].length ? [...new Set(combined)] : [fallback];
};
const firstImage = (item, fallback) => itemImages(item, fallback)[0];
...
function render(){
  ...
  $('[data-posts]').innerHTML=posts.length? [...posts].sort((a,b)=>new Date(b.publishedAt)-new Date(a.publishedAt)).slice(0,6).map(p=>{
    const images=itemImages(p,'assets/images/blog-workshop.svg');
    return `<article class="blog-card reveal"><div class="blog-image-wrap"><img src="${images[0]}" alt="${escapeHtml(localized(p.title))}">${images.length>1?`<span class="image-count-pill">${photoLabel(images.length)}</span>`:''}</div><div class="blog-body"><div class="blog-date">${formatDate(p.publishedAt)}</div><h3>${escapeHtml(localized(p.title))}</h3><p>${escapeHtml(localized(p.excerpt))}</p><button class="blog-link" type="button" data-open-post="${escapeHtml(p.id)}">${tr('readPost')} ↗</button></div></article>`;
  }).join(''):`<div class="empty">${tr('emptyPosts')}</div>`;
...
function setupProducts(products){
  ...
  const draw=key=>{
    const list=key==='all'?products:products.filter(p=>p.categoryKey===key);
    $('[data-products]').innerHTML=list.length?[...list].sort((a,b)=>Number(b.featured)-Number(a.featured)).map(productCard).join(''):`<div class="empty">${tr('emptyProducts')}</div>`;
    bindProductCardActions();
  };
  draw('all');
  $$('[data-filter]').forEach(btn=>btn.onclick=()=>{$$('[data-filter]').forEach(b=>b.classList.remove('active'));btn.classList.add('active');draw(btn.dataset.filter)});
}
function productCard(p){
  const images=itemImages(p);
  const dots=images.length>1?`<div class="product-dots">${images.map((_,i)=>`<span class="${i===0?'active':''}"></span>`).join('')}</div>`:'';
  const nav=images.length>1?`<div class="product-mini-nav"><button type="button" data-card-prev aria-label="Previous">‹</button><button type="button" data-card-next aria-label="Next">›</button></div>`:'';
  return `<article class="product-card reveal" data-product-card="${escapeHtml(p.id)}" data-card-index="0"><div class="product-image"><button class="product-gallery-trigger" type="button" data-open-product="${escapeHtml(p.id)}" aria-label="${escapeHtml(tr('viewGallery'))}"><img data-card-image src="${images[0]}" alt="${escapeHtml(localized(p.title))}"></button>${p.featured?`<span class="badge">${tr('featured')}</span>`:''}${images.length>1?`<span class="image-count-pill">${photoLabel(images.length)}</span>`:''}${nav}${dots}</div><div class="product-body"><small>${escapeHtml(localized(p.category))}</small><h3>${escapeHtml(localized(p.title))}</h3><p>${escapeHtml(localized(p.description))}</p><button class="gallery-link" type="button" data-open-product="${escapeHtml(p.id)}">${tr('viewGallery')} <span>↗</span></button></div></article>`;
}
function bindProductCardActions(){
  $$('[data-open-product]').forEach(btn=>btn.onclick=e=>{e.preventDefault();openProductGallery(btn.dataset.openProduct)});
  $$('[data-product-card]').forEach(card=>{
    const id=card.dataset.productCard;
    const item=(SITE_DATA.products||[]).find(p=>p.id===id);
    const images=itemImages(item);
    const move=delta=>{
      let index=Number(card.dataset.cardIndex||0);
      index=(index+delta+images.length)%images.length;
      card.dataset.cardIndex=String(index);
      $('[data-card-image]',card).src=images[index];
      $$('.product-dots span',card).forEach((dot,i)=>dot.classList.toggle('active',i===index));
    };
    const prev=$('[data-card-prev]',card),next=$('[data-card-next]',card);
    if(prev)prev.onclick=e=>{e.stopPropagation();move(-1)};
    if(next)next.onclick=e=>{e.stopPropagation();move(1)};
  });
}
function openProductGallery(id,index=0){
  const item=(SITE_DATA.products||[]).find(p=>p.id===id); if(!item)return;
  ACTIVE_PRODUCT_GALLERY={item,images:itemImages(item),index};
  renderProductGallery();
  $('[data-product-gallery-modal]').classList.add('open');
}
function renderProductGallery(){
  const {item,images}=ACTIVE_PRODUCT_GALLERY; if(!item)return;
  const index=(ACTIVE_PRODUCT_GALLERY.index+images.length)%images.length; ACTIVE_PRODUCT_GALLERY.index=index;
  $('[data-product-gallery-image]').src=images[index];
  $('[data-product-gallery-title]').textContent=localized(item.title);
  $('[data-product-gallery-category]').textContent=localized(item.category);
  $('[data-product-gallery-description]').textContent=localized(item.description);
  $('[data-product-gallery-count]').textContent=`${index+1} / ${images.length}`;
  $('[data-product-gallery-thumbs]').innerHTML=images.map((src,i)=>`<button class="gallery-thumb ${i===index?'active':''}" type="button" data-gallery-thumb="${i}"><img src="${src}" alt=""></button>`).join('');
  $$('[data-gallery-thumb]').forEach(btn=>btn.onclick=()=>{ACTIVE_PRODUCT_GALLERY.index=Number(btn.dataset.galleryThumb);renderProductGallery()});
  $$('[data-product-gallery-step]').forEach(btn=>btn.classList.toggle('hidden',images.length<2));
}
function stepProductGallery(delta){ACTIVE_PRODUCT_GALLERY.index+=delta;renderProductGallery()}
function openPost(id){
  const post=(SITE_DATA.posts||[]).find(p=>p.id===id);if(!post)return;
  ACTIVE_POST_GALLERY={item:post,images:itemImages(post,'assets/images/blog-workshop.svg'),index:0};
  $('[data-post-modal-title]').textContent=localized(post.title);
  $('[data-post-modal-content]').textContent=localized(post.content);
  renderPostGallery();
  $('[data-post-modal]').classList.add('open');
}
function renderPostGallery(){
  const {images}=ACTIVE_POST_GALLERY; if(!images.length)return;
  const index=(ACTIVE_POST_GALLERY.index+images.length)%images.length; ACTIVE_POST_GALLERY.index=index;
  $('[data-post-modal-image]').src=images[index];
  $('[data-post-gallery-count]').textContent=`${index+1} / ${images.length}`;
  $('[data-post-modal-thumbs]').innerHTML=images.map((src,i)=>`<button class="gallery-thumb ${i===index?'active':''}" type="button" data-post-gallery-thumb="${i}"><img src="${src}" alt=""></button>`).join('');
  $$('[data-post-gallery-thumb]').forEach(btn=>btn.onclick=()=>{ACTIVE_POST_GALLERY.index=Number(btn.dataset.postGalleryThumb);renderPostGallery()});
...
$$('[data-post-gallery-step]').forEach(btn=>btn.onclick=()=>stepPostGallery(Number(btn.dataset.postGalleryStep)));
$$('[data-product-gallery-close]').forEach(btn=>btn.onclick=()=> $('[data-product-gallery-modal]').classList.remove('open'));
$('[data-product-gallery-modal]').onclick=e=>{if(e.target.hasAttribute('data-product-gallery-modal'))e.currentTarget.classList.remove('open')};
$$('[data-product-gallery-step]').forEach(btn=>btn.onclick=()=>stepProductGallery(Number(btn.dataset.productGalleryStep)));
document.addEventListener('keydown',e=>{
  if(e.key==='Escape'){$('[data-post-modal]').classList.remove('open');$('[data-product-gallery-modal]').classList.remove('open')}
  if($('[data-product-gallery-modal]').classList.contains('open')&&e.key==='ArrowLeft')stepProductGallery(-1);
  if($('[data-product-gallery-modal]').classList.contains('open')&&e.key==='ArrowRight')stepProductGallery(1);
});
